...
@section('content')
    <div class="container">
        ...
        <form method="POST" action="{{ route('sales.userupdate', $sale->id) }}">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label for="number" class="form-label">Number</label>
                <input type="text" class="form-control" id="number" name="number" value="{{ $sale->number }}" readonly>
            </div>
            <div class="mb-3">
                ...
                <select class="form-control" id="inventory_id" name="inventory_id">
                    ...
                    @foreach ($inventories as $inventory)
                        <option value="{{ $inventory->id }}" data-price="{{ $inventory->price }}"
                            {{ $sale->inventory_id == $inventory->id ? 'selected' : '' }}>
